...
<form id="form" method="POST" enctype='multipart/form-data'>
	...
	<select required name="input_form_support_type_id" class="select2" data-label="support_type" <?=checkError('support_type', $input_forms_verify, $_GET['id'])?>>
		...
		foreach ($tbs_input_forms_supports_types as $types) {
			?>
			<option value="<?=$types['id']?>" <?=($input_form_support['input_form_support_type_id'] == $types['id'] ? 'selected' : '')?>><?=$types['id']?> - <?=$types['name']?></option>
			<?php
		} ?>
	</select>
	...
	<label>Fecha</label>
	<?=$calendar->render('created_at', $input_form_support['created_at'], TRUE, 'YYYY-MM-DD', $class.' data-label="created_at"')?>
	
	<label>Detalles</label>
	<textarea name="details" rows="5" data-label="details" <?=checkError('details', $input_forms_verify, $_GET['id'])?>><?=$input_form_support['details']?></textarea>

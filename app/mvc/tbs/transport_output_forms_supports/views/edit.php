...
<form id="form" method="POST" enctype='multipart/form-data'>
	...
	<label>Tipo</label>
	<select required name="transport_output_form_support_type_id" class="select2" data-label="support_type" <?=checkError('support_type', $output_forms_verify, $_GET['id'])?>>
		<option></option>
		<?php
		foreach ($output_forms_supports_types as $types) {
			?>
			<option value="<?=$types['id']?>" <?=($transport_output_form_support['transport_output_form_support_type_id'] == $types['id'] ? 'selected' : '')?>><?=$types['id']?> - <?=$types['name']?></option>
			<?php
		} ?>
	</select>

	<label>Soporte</label>
	<input name=""> type="file" name="support">

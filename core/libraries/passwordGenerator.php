...
class passwordGenerator
{
	...
	public function generate(): string{
		...
		$chars = self::LETTERS . strtoupper(self::LETTERS) . self::DIGITS . self::SPECIAL_CHARS;
		...
		while (!$passwordReady) {
			...
			while ($length > 0) {
				...
				$hasLowercase = $hasLowercase || (strpos(self::LETTERS, $char) !== false);
				$hasUppercase = $hasUppercase || (strpos(strtoupper(self::LETTERS), $char) !== false);
				$hasDigit = $hasDigit || (strpos(self::DIGITS, $char) !== false);
				$hasSpecialChar = $hasSpecialChar || (strpos(self::SPECIAL_CHARS, $char) !== false);
			}
